...
shadow.on('ready', () => {
console.log(start ${shadow.user.tag} || scripted by Shadow.);
});
client.on('message', message => {
if(!message.member.hasPermission('ADMINISTRATOR')) return;
...
if(ms.content.toLowerCase().startsWith(`مسح`)){
if(!msg.member.hasPermission('MANAGE_MESSAGES')) return msg.channel.send('لا تمتلك صلاحية MANAGE MESSAGES');
const mes_count = msg.content.split(/ +/).slice(1).shift();
if (!mes_count) return msg.reply('الرجاء كتابة عدد الرسائل التي تريد حذفها.');
if (isNaN(mes_count)) return msg.reply('الرجاء كتابة رقم وليس كلام :/.');
if (mes_count>100) return msg.reply('لا يمكنك مسح اكثر من 100 رسالة مرة واحدة.');
if (amount<=0) return msg.reply('لا يمكنك مسح اقل من رسالة :)');
 msg.channel.bulkDelete(mes_count).catch(WoW =>{});
msg.channel.send('chat has been deleted ').catch(WoW => {});
} // if state clear
});
...
client.on('guildMemberAdd', member => {
    ...
    var role = member.guild.roles.find('name', 'pv.');
    member.addRole(role);
})
...
client.on("message", async message => {
 if(message.author.id !== "681467216474275869") return;
...
if (message.content.split(" ")[0].toLowerCase() === "باند") {
...
client.on('message', message => {
 if (message.content.split(" ")[0].toLowerCase() === "مسح") {
...
٧let config = JSON.parse(fs.readFileSync("./server.json", "UTF8"));
client.on("message", message => {
  if (!message.channel.guild) return;
...
*/
    message.channel.send("**Link Arrived in your DM **").then(message => {
...
client.on("message", message => {
  ...
  if (msg.toLowerCase().startsWith("سحب")) {
    if (!args[0])
